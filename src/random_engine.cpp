...
UniformSampler<RandomEngine>::UniformSampler(size_t maxSize, size_t dim) :maxSize(maxSize), dim(dim) {
    RandomCreator = std::make_shared<RandomEngine>(RandomEngine(maxSize, dim));
}


void testSobolSampler() {
    UniformSampler<LdsGenerator> sampler(1, 1);
}